...
<ui:html3>
    ...
    <ui:navbar active="" action="${action}"/>
        ...
        <div class="comments" >
            ...
            <div class="head">
                ...
                <div class="right-head-container">
                    ...
                    <div class="second-row"> <fmt:message key="task"/>: &nbsp #${task.taskId} ${task.summary}</div>
                </div>
            </div>
                                ...
                                <div class="comment-body">  
                                    <div class="first-row">
                                        <sec:authorize access="hasRole('admin')">
                                            ...
                                            <c:url value="${url_del}?cid=${comment.id}&tid=${task.id}&login=${comment.login}" var="href" />
                                        ...
                                        <sec:authorize access="!hasRole('admin')">
                                            <c:choose>
                                                <c:when test="${uid eq comment.userId}">
                                                    ${comment.name} &nbsp | &nbsp <fmt:formatDate type="both" pattern="yyyy-MM-dd HH:mm" value="${comment.dateComment}"/> &nbsp | &nbsp 
                                                    <c:url value="${url_del}?cid=${comment.id}&tid=${task.id}" var="href" />
                                                    <a href="${href}"><fmt:message key="comment.del"/></a>
                                                </c:when>
                                                <c:otherwise>
